import * as React from "react";
import Typography from "@mui/material/Typography";
import Grid from "@mui/material/Grid";
import Divider from "@mui/material/Divider";

export default function Experience() {
  return (
    <>
    <Typography variant="h3" align="center" margin="6rem 0 3rem">Experience</Typography>
    <Grid container justifyContent="space-evenly">
      <Grid item >
      <Typography variant="h5" align="center" sx={{ mb: 2 }}>Work</Typography>
        <Grid item >
          
          <Typography> Self-employed at FIK design courses</Typography>
          <Typography>2017 – now </Typography>
          <Typography>
            Preparatory interior architecture & design course
          </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>
            BOAB, Hällefors Bostads AB, Housing Association
          </Typography>
          <Typography>2019 – 2021 </Typography>
          <Typography>Chairman</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Pihlskolan, Hällefors</Typography>
          <Typography>2019 – 2020</Typography>
          <Typography>Teacher SFI, Swedish for immigrants</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Pihlskolan, Hällefors</Typography>
          <Typography>2016 – 2019</Typography>
          <Typography>Teacher SVA, Swedish as a second language</Typography>
          <Typography>High School education</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Hällefors Folkhögskola</Typography>
          <Typography>2008 – 2015 </Typography>
          <Typography>Head teacher, Interior Architecture & Design</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Hällefors Folkhögskola</Typography>
          <Typography>2007 – 2008 </Typography>
          <Typography>Teacher, Form & Design</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Hoganord Studios</Typography>
          <Typography>2004 – 2009 </Typography>
          <Typography>
            Co-owner and designer, Graphic design & Web design
          </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>KUUB design, Amsterdam</Typography>
          <Typography>2001 – 2003</Typography>
          <Typography>Co-owner and interior designer </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item > 
          <Typography>De Architect, Dutch architecture magazine</Typography>
          <Typography>2001 – 2005</Typography>
          <Typography>Freelance writer</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
      </Grid>
      

      <Grid item>
      <Typography variant="h5" align="center" sx={{ mb: 2 }}>Education</Typography>
        <Grid item >
          <Typography>Sundsgården folkhögskola</Typography>
          <Typography>2023 </Typography>
          <Typography>
            Tech for Women, Front end development, Certification
          </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>freeCodeCamp</Typography>
          <Typography>2023</Typography>
          <Typography>Responsive Webdesign Developer Certification</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>KBT Svealand AB </Typography>
          <Typography>Utbildningcenter för Kognitiv Beteendeterapi</Typography>
          <Typography>2021</Typography>
          <Typography>
            Introduction Course CBT, Cognitive Behavioral Therapy
          </Typography>
          <Typography>Certification</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>
            Gerrit Rietveld Academy, Amsterdam, Netherlands
          </Typography>
          <Typography>1997 – 2001</Typography>
          <Typography>
            Bachelor's degree in Interior Architecture & Spatial Design
          </Typography>
          <Typography>240 credits, Cum Laude - With Honor</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>
            Royal Academy of Arts in The Hague, Netherlands
          </Typography>
          <Typography>1992 – 1996</Typography>
          <Typography>Bachelor's degree in Free Arts, 240 credits</Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Linköping University, Sweden</Typography>
          <Typography>2010</Typography>
          <Typography>
            Folkhögskolan's idea and pedagogy, 7.5 credits
          </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
        <Grid item >
          <Typography>Creative Coaching Association, USA</Typography>
          <Typography>2012</Typography>
          <Typography>
            Creative coaching training, 16 weeks, Certification
          </Typography>
        </Grid>
        <Divider variant="middle" sx={{ mb: 4, mt: 4}}/>
      </Grid>
    </Grid>
    </>
  );
}
